...
@Bot.on_callback_query(filters.regex('^add_admin$')& (filters.user(get_admin()) | filters.user(SUDO_USERS)))
async def add_admin_handler(bot:Client,message:Message):
    try:
        ...
        if add_admins.text=='🚫 Cancel':
            await bot.send_message(message.from_user.id,'Cancelled',reply_markup=admin_markup())
        else:
            add_admin(int(add_admins))
            LOGGER.info(f"{add_admins.text} added as admin by {message.from_user.id}")
            await bot.send_message(message.from_user.id,"Admin Added Sucessfully",reply_markup=admin_markup())
    except Exception as e:
            LOGGER.error(e)
            await bot.send_message(LOG_CHANNEL,f'\n<code>{traceback.format_exc()}</code>\n\nTime : {time.ctime()} UTC',parse_mode='html')
            await bot.send_message(message.from_user.id,'Aww :( , Something went wrong',reply_markup=admin_markup())
